...
class _FoodDetailScreenState extends State<FoodDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(food.formattedName, overflow: TextOverflow.ellipsis)),
      body: ListView(
        ...
        children: [
          ...
          Text(food.displayName, style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
          if (food.brand != null)
            Text(food.brand!, style: theme.textTheme.bodyMedium?.copyWith(color: theme.colorScheme.onSurfaceVariant)),
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: TextField(
                          ...
                          decoration: const InputDecoration(suffix: Text('g'), border: OutlineInputBorder()),
                      ...
                      if (food.servingGrams != null)
                        TextButton(
                          onPressed: () {
                            _gramsController.text = food.servingGrams!.toStringAsFixed(0);
                            setState(() {});
                          },
                          child: Text('1 serving (${food.servingGrams!.toStringAsFixed(0)}g)'),
                        ),
                    ],
                  ),
                  ..._buildPortionChips(food),
                  const SizedBox(height: 8),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Wrap(
                      spacing: 8,
                      children: Meal.values.map((m) => ChoiceChip(
                        label: Text(m.label),
                        selected: _meal == m,
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Nutrition (per ${_grams.toStringAsFixed(0)}g)', style: theme.textTheme.titleSmall),
                  const SizedBox(height: 12),
                  _NutrientRow('Calories', '${_calories.toStringAsFixed(0)} kcal', bold: true),
                  _NutrientRow('Carbohydrates', '${food.carbsForGrams(_grams).toStringAsFixed(1)}g'),
                  _NutrientRow('  of which sugars', '${(food.sugarsPer100g * _grams / 100).toStringAsFixed(1)}g', indent: true),
                  _NutrientRow('Fat', '${food.fatForGrams(_grams).toStringAsFixed(1)}g'),
                  _NutrientRow('  of which saturates', '${(food.saturatedFatPer100g * _grams / 100).toStringAsFixed(1)}g', indent: true),
                  _NutrientRow('Protein', '${food.proteinForGrams(_grams).toStringAsFixed(1)}g'),
                  _NutrientRow('Fibre', '${(food.fiberPer100g * _grams / 100).toStringAsFixed(1)}g'),
                  _NutrientRow('Sodium', '${(food.sodiumPer100g * _grams / 100 * 1000).toStringAsFixed(0)}mg'),
                ],
              ),
            ),
          ),
...
class _NutrientRow extends StatelessWidget {
  ...
  const _NutrientRow(this.label, this.value, {this.bold = false, this.indent = false});
  ...
  Widget build(BuildContext context) {
    final style = bold
        ...
        : Theme.of(context).textTheme.bodyMedium?.copyWith(
              color: indent ? Theme.of(context).colorScheme.onSurfaceVariant : null,
            );
    return Padding(
