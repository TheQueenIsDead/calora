...
class _FoodDetailScreenState extends State<FoodDetailScreen>
  ...
  void initState() {
    ...
    final defaultGrams =
        widget.initialGrams ?? widget.food.servingGrams ?? 100.0;
  ...
  static List<({String label, double grams})> _portionSuggestions(String name) {
    ...
    if (has('kiwifruit') ||
        has('kiwi fruit') ||
        (has('kiwi') && !has('chicken'))) {
      return [(label: '1 fruit', grams: 76), (label: '2 fruit', grams: 152)];
    }
    if (has('apple')) {
      return [
        (label: 'Small', grams: 130),
        (label: 'Medium', grams: 185),
        (label: 'Large', grams: 240),
      ];
    }
    if (has('pear')) {
      return [
    ...
    if (has('mandarin') || has('clementine') || has('satsuma')) {
      return [(label: '1 fruit', grams: 75), (label: '2 fruit', grams: 150)];
    }
    if (has('avocado')) {
      return [
        (label: '½ avocado', grams: 85),
        (label: '1 avocado', grams: 170),
      ];
    }
    if (has('mango')) {
      return [(label: '½ cup', grams: 83), (label: '1 cup', grams: 165)];
    }
    if (has('peach') || has('nectarine') || has('plum') || has('apricot')) {
      return [(label: '1 fruit', grams: 130), (label: '2 fruit', grams: 260)];
    }
    if (has('grape')) {
      return [(label: 'Small bunch', grams: 80), (label: '1 cup', grams: 150)];
    }
    if (has('strawberr') || has('blueberr') || has('raspberr')) {
      return [(label: 'Handful', grams: 80), (label: '1 cup', grams: 150)];
    }
    if (has('watermelon')) {
      return [(label: '1 slice', grams: 280), (label: '1 cup', grams: 154)];
    }
    if (has('pineapple')) {
      return [(label: '1 ring', grams: 84), (label: '1 cup', grams: 165)];
    }
    if (has('lemon') || has('lime')) {
      return [(label: '1 fruit', grams: 58), (label: '½ fruit', grams: 29)];
    }

    // Vegetables
    if (has('potato') && !has('sweet')) {
      return [
    ...
    if (has('sweet potato') || has('kumara')) {
      return [
        (label: 'Small', grams: 130),
        (label: 'Medium', grams: 180),
    ...
    if (has('carrot')) {
      return [(label: '1 medium', grams: 61), (label: '2 medium', grams: 122)];
    }
    if (has('broccoli')) {
      return [(label: '1 cup', grams: 91), (label: '½ head', grams: 200)];
    }
    if (has('cauliflower')) {
      return [(label: '1 cup', grams: 107), (label: '½ head', grams: 300)];
    }
    if (has('tomato')) {
      return [
        (label: 'Small', grams: 90),
        (label: 'Medium', grams: 130),
        (label: 'Large', grams: 180),
      ];
    }
    if (has('capsicum') || has('bell pepper')) {
      return [
        (label: '½ capsicum', grams: 60),
        (label: '1 capsicum', grams: 120),
      ];
    }
    if (has('onion')) {
      return [
    ...
    if (has('corn') || has('sweetcorn')) {
      return [(label: '½ cob', grams: 90), (label: '1 cob', grams: 180)];
    }
    if (has('cucumber')) {
      return [
        (label: '½ cucumber', grams: 150),
        (label: '1 cucumber', grams: 300),
      ];
    }
    if (has('zucchini') || has('courgette')) {
      return [(label: '½ medium', grams: 100), (label: '1 medium', grams: 196)];
    }
    if (has('eggplant') || has('aubergine')) {
      return [
        (label: '½ eggplant', grams: 200),
        (label: '1 eggplant', grams: 400),
      ];
    }
    if (has('pumpkin')) {
      return [
        (label: '1 cup cubed', grams: 116),
        (label: '2 cups cubed', grams: 232),
      ];
    }
    if (has('mushroom')) {
      return [(label: '1 cup', grams: 96), (label: '4 medium', grams: 72)];
    }
    if (has('spinach') || has('silverbeet')) {
      return [
        (label: '1 cup raw', grams: 30),
        (label: '½ cup cooked', grams: 90),
      ];
    }
    if (has('lettuce') || has('salad green')) {
      return [(label: '1 cup', grams: 47), (label: '2 cups', grams: 94)];
    }
    if (has('celery')) {
      return [(label: '1 stalk', grams: 40), (label: '3 stalks', grams: 120)];
    }
    if (has('asparagus')) {
      return [(label: '4 spears', grams: 60), (label: '8 spears', grams: 120)];
    }
    if (has('pea')) {
      return [(label: '½ cup', grams: 80), (label: '1 cup', grams: 160)];
    }
    if (has('bean') && (has('green') || has('string'))) {
      return [(label: '½ cup', grams: 55), (label: '1 cup', grams: 110)];
    }

    // Grains
    if (has('bread') || has('toast')) {
      return [
        (label: '1 thin slice', grams: 25),
        (label: '1 slice', grams: 35),
        (label: '1 thick slice', grams: 45),
      ];
    }
    if (has('rice') && !has('rice cake')) {
      return [
        (label: '½ cup', grams: 100),
    ...
    if (has('pasta') ||
        ...
        has('macaroni') ||
        has('fettuccine')) {
      return [(label: '1 cup', grams: 140), (label: '1½ cups', grams: 210)];
    }
    if (has('noodle')) {
      return [(label: '1 cup', grams: 140), (label: '2 cups', grams: 280)];
    }
    if (has('oat') || has('porridge') || has('muesli')) {
      return [
        (label: '½ cup dry', grams: 45),
        (label: '1 cup cooked', grams: 250),
      ];
    }
    if (has('wrap') || has('tortilla')) {
      return [(label: '1 small', grams: 35), (label: '1 large', grams: 50)];
    }

    // Protein
    if (has('egg')) {
      return [
        (label: '1 medium', grams: 58),
        (label: '2 medium', grams: 116),
        (label: '3 medium', grams: 174),
      ];
    }
    if (has('chicken breast')) {
      return [
        (label: 'Small', grams: 120),
        (label: 'Medium', grams: 170),
        (label: 'Large', grams: 230),
      ];
    }
    if (has('chicken thigh')) {
      return [
        (label: 'Small', grams: 90),
        (label: 'Medium', grams: 130),
        (label: 'Large', grams: 180),
    ...
    if (has('chicken') && has('drumstick')) {
      return [
        (label: '1 drumstick', grams: 100),
        (label: '2 drumsticks', grams: 200),
      ];
    }
    if (has('salmon')) {
      return [
        ...
        (label: 'Large fillet', grams: 200),
      ];
    }
    if (has('tuna')) {
      return [(label: '½ can', grams: 95), (label: '1 can', grams: 185)];
    }
    if (has('beef') || has('steak')) {
      return [
        (label: 'Small', grams: 100),
        (label: 'Medium', grams: 150),
        (label: 'Large', grams: 200),
      ];
    }
    if (has('lamb chop') || has('pork chop')) {
      return [(label: '1 chop', grams: 100), (label: '2 chops', grams: 200)];
    }
    if (has('tofu')) {
      return [(label: '½ cup', grams: 126), (label: '1 cup', grams: 252)];
    }
    if (has('lentil') ||
        has('chickpea') ||
        (has('bean') && !has('coffee') && !has('green'))) {
      return [
        (label: '½ cup cooked', grams: 100),
        (label: '1 cup cooked', grams: 200),
      ];
    }
    if (has('peanut butter') || has('almond butter')) {
      return [(label: '1 tbsp', grams: 16), (label: '2 tbsp', grams: 32)];
    }
    if (has('almond') ||
        has('cashew') ||
        has('walnut') ||
      ...
      return [
        ...
        (label: 'Large handful', grams: 40),
      ];
    }

    // Dairy
    if (has('milk')) {
      return [(label: '½ cup', grams: 125), (label: '1 cup', grams: 250)];
    }
    if (has('cheese') && !has('cheesecake')) {
      return [
        (label: '1 slice', grams: 20),
        (label: 'Thick slice', grams: 30),
        (label: '¼ cup grated', grams: 30),
      ];
    }
    if (has('yoghurt') || has('yogurt')) {
      return [
        (label: 'Small pot', grams: 100),
        (label: 'Large pot', grams: 170),
      ];
    }
    if (has('butter') || has('margarine')) {
      return [(label: '1 tsp', grams: 5), (label: '1 tbsp', grams: 14)];
    }
                ...
                Row(
                  children: [
                    ...
                    if (food.servingGrams != null)
                      TextButton(
                        onPressed: () {
                          gramsController.text = food.servingGrams!
                              .toStringAsFixed(0);
                          onGramsChanged();
                        },
                        child: Text(
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Nutrition per ${g.toStringAsFixed(0)}g',
                ...
                _NutrientRow(
                  'Fat',
                  '${food.fatForGrams(g).toStringAsFixed(1)}g',
                ),
                _NutrientRow(
                  '  of which saturates',
                ...
                _NutrientRow(
                  'Fibre',
                  '${(food.fiberPer100g * g / 100).toStringAsFixed(1)}g',
                ),
                _NutrientRow(
                  'Sodium',
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                _NutrientRow('Fat', '${food.fatPer100g.toStringAsFixed(1)}g'),
                _NutrientRow(
                  '  of which saturates',
