...
void main() {
  ...
  test('getFoodById: returns null for unknown id', () async {
    final result = await DatabaseService.instance.getFoodById('no_such_food_xyz_000');
    expect(result, isNull);
  });

  // ── Goal history ──────────────────────────────────────────────────────────

  test('saveGoal and getEffectiveGoal: returns saved value for same date', () async {
    await DatabaseService.instance.saveGoal(DateTime(2098, 1, 1), 2200);
    final result = await DatabaseService.instance.getEffectiveGoal(DateTime(2098, 1, 1));
    expect(result, 2200);
  });

  test('getEffectiveGoal: step-function — applies most-recent goal on or before date', () async {
    ...
    final mid = await DatabaseService.instance.getEffectiveGoal(DateTime(2098, 2, 5));
    expect(mid, 1800);

    // After the second change → sees 2400
    final after = await DatabaseService.instance.getEffectiveGoal(DateTime(2098, 2, 15));
    expect(after, 2400);
  });

  test('getEffectiveGoal: returns null when no goal set before date', () async {
    final result = await DatabaseService.instance.getEffectiveGoal(DateTime(1990, 1, 1));
    expect(result, isNull);
  });
  ...
  test('saveWeight and getWeightHistory: round-trip persists the entry', () async {
    await DatabaseService.instance.saveWeight(DateTime(2098, 4, 1), 75.5);
    final history = await DatabaseService.instance.getWeightHistory(days: 365 * 200);
    expect(
      history.any((e) => e.date.year == 2098 && e.date.month == 4 && (e.kg - 75.5).abs() < 0.01),
      isTrue,
    );
  });

  test('saveWeight: upserts — second save on same date replaces first', () async {
    ...
    final entries = history
        .where((e) => e.date.year == 2098 && e.date.month == 4 && e.date.day == 2)
        .toList();
    expect(entries.length, 1);
    expect(entries.first.kg, closeTo(72.3, 0.01));
  });

  test('getLatestWeight: returns a non-null entry after weights are saved', () async {
  ...
  test('deleteWeight: removes the entry so it no longer appears in history', () async {
    ...
    final after = await DatabaseService.instance.getWeightHistory(days: 365 * 200);
  ...
  test('updateEntryMeal: changes the meal column in DB', () async {
    final food = _food();
    final entry = _entry(food, date: DateTime(2099, 3, 2), meal: Meal.breakfast);
    await DatabaseService.instance.addDiaryEntry(entry);
  ...
  test('getDailyCalories: aggregates kcal per day across multiple entries', () async {
    ...
    await DatabaseService.instance.addDiaryEntry(_entry(food, date: dateA, grams: 100));
    await DatabaseService.instance.addDiaryEntry(_entry(food, date: dateA, grams: 50));
    await DatabaseService.instance.addDiaryEntry(_entry(food, date: dateB, grams: 200));

    final result = await DatabaseService.instance.getDailyCalories(dateA, dateB);
  ...
  test('getDailyMacros: aggregates protein/fat/carbs per day', () async {
    ...
    final food = _food(id: const Uuid().v4(), calories: 300, protein: 20, fat: 10, carbs: 40);
    final date = DateTime(2099, 4, 10);
    await DatabaseService.instance.addDiaryEntry(_entry(food, date: date, grams: 200));
  ...
  test('getRecentFoods: includes foods logged within the last 30 days', () async {
  ...
  test('getLastMealFoods: returns foods from the most recent prior occurrence of that meal', () async {
  ...
  test('saveRecipeWithItems and getRecipeAsFood: computes macros proportionally', () async {
    // 500 kcal/100g, protein=30, fat=20, carbs=50 per 100g
    // 200g of this food → 1000 kcal total → still 500 kcal/100g normalised
    final food = _food(id: const Uuid().v4(), calories: 500, protein: 30, fat: 20, carbs: 50);
    await DatabaseService.instance.saveFood(food);
  ...
  test('getRecipesAsFood: lists all recipes and supports name filtering', () async {
    ...
    final filtered = await DatabaseService.instance.getRecipesAsFood('QueryableRecipe');
    expect(filtered.any((r) => r.name == 'QueryableRecipe_Alpha'), isTrue);

    final none = await DatabaseService.instance.getRecipesAsFood('zzznomatch_xyz_000');
  ...
  test('deleteRecipe: removes recipe and all its items atomically', () async {
    final food = _food(id: const Uuid().v4(), calories: 100);
    await DatabaseService.instance.saveFood(food);
    final recipeId = await DatabaseService.instance.saveRecipe('ToDelete', null);
    await DatabaseService.instance.addRecipeItem(recipeId, food.id, 100);
  ...
  test('addRecipeItem / updateRecipeItemGrams / deleteRecipeItem: full item CRUD', () async {
    final food = _food(id: const Uuid().v4(), calories: 100);
    await DatabaseService.instance.saveFood(food);
    final recipeId = await DatabaseService.instance.saveRecipe('ItemCrudRecipe', null);
  ...
  test('getRecipeItems: returns enriched rows with food name and macros', () async {
    final food = _food(id: const Uuid().v4(), name: 'IngredientFood', calories: 100);
    await DatabaseService.instance.saveFood(food);
    final recipeId = await DatabaseService.instance.saveRecipe('EnrichedRecipe', null);
  ...
  test('updateRecipeServings: changes servings so servingGrams is divided correctly', () async {
    final food = _food(id: const Uuid().v4(), calories: 400);
    await DatabaseService.instance.saveFood(food);
